...
CLASS lcl_mermaid DEFINITION INHERITING FROM lcl_popup FRIENDS  lcl_debugger_script.
  ...
  PUBLIC SECTION.
    ...
    METHODS: constructor IMPORTING io_debugger TYPE REF TO lcl_debugger_script
      ...
      parse_call IMPORTING iv_program TYPE program iv_index TYPE i iv_stack TYPE i iv_event TYPE string,
      add_toolbar_buttons,
...
CLASS lcl_window DEFINITION INHERITING FROM lcl_popup .
  ...
  PUBLIC SECTION.
    ...
    METHODS set_program_line IMPORTING iv_line LIKE sy-index OPTIONAL.
...
CLASS lcl_event_handler IMPLEMENTATION.
  ...
  METHOD on_editor_border_click.
    ...
    LOOP AT mo_debugger->mo_window->mt_bpoints ASSIGNING FIELD-SYMBOL(<point>) WHERE line = line.
      lv_type = <point>-type.

      CALL FUNCTION 'RS_DELETE_BREAKPOINT'
        EXPORTING
          index        = line
          mainprog     = mo_debugger->mo_window->m_prg-program
          program      = mo_debugger->mo_window->m_prg-include
          bp_type      = lv_type
        EXCEPTIONS
          not_executed = 1
          OTHERS       = 2.
    ...
    IF sy-subrc <> 0. "create
      CALL FUNCTION 'RS_SET_BREAKPOINT'
        EXPORTING
          index        = line
          program      = mo_debugger->mo_window->m_prg-include
          mainprogram  = mo_debugger->mo_window->m_prg-program
          bp_type      = lv_type
        EXCEPTIONS
          not_executed = 1
          OTHERS       = 2.
    ...
    ENDIF.
    DELETE mo_debugger->mo_window->mt_bpoints WHERE del IS NOT INITIAL.
    mo_debugger->mo_window->set_program_line( ).
  ENDMETHOD.
  ...
  METHOD magic_search.
    ...
    LOOP AT lt_steps ASSIGNING FIELD-SYMBOL(<step>).
      ...
      IF lv_prev IS NOT INITIAL.
        IF ( ls_key-name = 'ENDDO' OR ls_key-name = 'ENDWHILE' OR ls_key-name = 'ENDLOOP' OR ls_key-name = 'ENDIF' )  AND
           ( lv_prev_key = 'DO' OR lv_prev_key = 'LOOP'  OR lv_prev_key = 'WHILE'  OR lv_prev_key = 'IF' ).
...
*          CLEAR <line>-els_before.
  ...
  METHOD parse_call.
    ...
    READ TABLE mo_debugger->mo_window->mt_source WITH KEY include = iv_program INTO DATA(ls_source).
    DO.
      ...
      IF ls_key-to_evname IS NOT INITIAL.
        ...
        parse_call( EXPORTING iv_index = ls_call_line-index
  ...
  METHOD code_execution_scanner.
    ...
    DATA: lt_str LIKE ls_source-scan->structures.

    READ TABLE ls_source-scan->structures WITH KEY type = 'E' TRANSPORTING  NO FIELDS.
    IF sy-subrc = 0.
      lt_str = ls_source-scan->structures.
      DELETE lt_str WHERE type <> 'E'.
      SORT lt_str BY stmnt_type ASCENDING.
    ELSE.
      CLEAR lv_max.
      LOOP AT ls_source-scan->structures INTO DATA(ls_str) WHERE type <> 'P' AND type <> 'C' .
    ...
    LOOP AT lt_str INTO ls_str.
      ...
      WHILE lv_statement <= ls_str-stmnt_to.
        ...
        <step>-stacklevel = lv_stack.
        <step>-program = ls_source-include.
        <step>-include = ls_source-include.
